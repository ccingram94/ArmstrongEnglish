      ...
      <div className="w-full min-h-screen bg-violet-900/90 text-center items-center">
        ...
        <div className="h-full flex flex-row flex-wrap justify-center p-2">
          <div className="flex flex-col h-full lg:w-1/2 text-center items-center justify-center">
            ...
            <p className="text-sm max-w-md p-2 m-2 font-bold text-white/90">{t('CEFR A1 is the first step in learning English.  Students can understand and use basic words and sentences for everyday conversations.')} </p>
            <div className="flex flex-wrap justify-center container mx-auto w-fit">
            { Object.keys(a1data).map((key, index) => (
                  <Link href={"/lesson/" + index} className="w-full md:w-fit bg-white/90 hover:bg-white transition-all rounded-md shadow-xl p-6 m-2" key={index}>
                    <p className="text-sm font-extrabold text-violet-900/80">
                    {t('Lesson')} { a1data[key].id }
                    </p>
                    <p className="text-xl font-extrabold text-violet-900">
                    { a1data[key].title }
                    </p>
                    <p className="text-sm font-bold text-violet-900/80">
                      {t(`${a1data[key].title}`)}
                    </p>
                  </Link>
              ))}
